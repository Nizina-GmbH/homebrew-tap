#!/usr/bin/env bash
# Veyloq one-line installer (macOS).
#
...
# com.apple.quarantine so the app launches (see apps/desktop/homebrew/veyloq.rb) —
# without it macOS shows "Veyloq is damaged / unidentified developer". The DMG is
...
#   https://raw.githubusercontent.com/Nizina-GmbH/homebrew-tap/main/install-veyloq.sh
...
CASK="veyloq"                  # cask token → installs Veyloq.app
# ──────────────────────────────────────────────────────────────────────────────
...
#    upgrades in place.
say "Installing Veyloq…"
brew install --cask "${TAP}/${CASK}"

ok "Veyloq installed. Launch it from Applications (or Spotlight)."
